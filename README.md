...
각각 시작 시간, 일시정지했을 때 경과 시간, 화면에 보여질 시간입니다.
...
일정 시간 후 경과 여부를 알려주기 위한 user input에 연결할 Ref와, 알람 컴포넌트가 보여질지 말지를 결정하는 상태입니다.
...
알람 컴포넌트의 값도 실시간으로 변경되는 문제가 있습니다. 적어도 사용자가 값 변경을 마친 후에 반응해야 한다고 생각해 비제어 input으로 두되, `targetSecs`의 상태 추적은 여전히 필요해 상태 업데이트 또한 진행하는 방향으로 구현했습니다.
...
- 시간 측정은 `performance.now()`로
  ...
  - `Date.now()`는 시스템 시계를 기준으로 하기 때문에 타이머가 동작하는 중간에 시스템 시간이 바뀌면 타이머의 동작성을 보장하지 못함 (사용자 임의 변경이나 자동 시간대 변경 기능 등)
- 1초보다 짧은 주기(100ms)로 시간을 측정해 UI상의 정확성 확보
  - 1000ms마다 측정하면 초 단위로 업데이트하는 특성상 최악의 경우 0.9초 딜레이
  - `requestAnimationFrame`도 고려했으나 매 프레임마다 리렌더하는 것은 초 단위로만 업데이트되는 타이머에서 과하다고 판단
